...
PROMPT_TOKEN_BUDGET = int(os.getenv("LLM_PROMPT_TOKEN_BUDGET", "10000"))
...
def _rule_prefilter(
    ...
    分级策略（确保候选池有真正的厂商一手信号，不被 awesome 列表挤掉）：
      Tier-1: **严格官方源**（厂商 / 产品官博 + 官方文档，不含 github/arxiv）—— 全部保留
      Tier-2: **arXiv 论文 + 厂商在 GitHub 上的官方仓库** —— 受 keyword 域名锚定的项目
      Tier-3: **多样性轮询**：剩余按 matched_keywords 轮询（每个跟踪关键词最多 N 条）
      Tier-4: 余下按 score 倒序填
    """
    # Tier-1: 严格官方源（厂商博客优先级最高）
    tier1 = [it for it in pool if _is_strictly_official(it, keywords)]
    tier1_ids = {it.get("id") for it in tier1 if it.get("id")}

    # Tier-2: github / arxiv 这种"准官方"全局白名单
    tier2 = [
        it for it in pool
        if _is_official_any(it, keywords)
        and it.get("id") not in tier1_ids
    ]
    tier2_ids = {it.get("id") for it in tier2 if it.get("id")} | tier1_ids

    # Tier-3 候选：剩下的非官方
    rest = [it for it in pool if it.get("id") not in tier2_ids]

    out: list[dict] = list(tier1)
    if len(out) >= keep_n:
        return sorted(out, key=lambda x: float(x.get("score", 0) or 0), reverse=True)[:keep_n]

    # 给 Tier-2 留约 1/3 名额，避免 github trending 列表压满
    remaining = keep_n - len(out)
    t2_quota = min(len(tier2), max(1, remaining // 3))
    tier2_sorted = sorted(tier2, key=lambda x: float(x.get("score", 0) or 0), reverse=True)
    out.extend(tier2_sorted[:t2_quota])

    remaining = keep_n - len(out)
    if remaining <= 0:
        return out

    # Tier-3 多样性轮询：每个 matched_keywords[0] 桶最多取 ceil(remaining/桶数)
    rest_sorted = sorted(rest, key=lambda x: float(x.get("score", 0) or 0), reverse=True)
    buckets: dict[str, list[dict]] = {}
    for it in rest_sorted:
        key = (it.get("matched_keywords") or ["_unk"])[0]
        buckets.setdefault(key, []).append(it)
    diversified: list[dict] = []
    while len(diversified) < remaining and any(buckets.values()):
        for key in list(buckets.keys()):
            if not buckets[key]:
                continue
            diversified.append(buckets[key].pop(0))
            if len(diversified) >= remaining:
                break
    out.extend(diversified)
    return out[:keep_n]
...
def rank_by_category(
    ...
    for cat in CATEGORIES:
        ...
        pool = buckets[cat][:max_pool]
        if not pool:
            print(f"  [LLM] {cat}: 0 条（候选池为空）")
            continue

        base_prompt = CATEGORY_PROMPT_TEMPLATE.format(
